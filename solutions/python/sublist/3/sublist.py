...
def sublist(list_one: list, list_two: list) -> Category:
    ...
    if len_one < len_two and any(
        list_one == list_two[i : i + len_one] for i in range(len_two - len_one + 1)
    ):
        return Category.SUBLIST

    if len_one > len_two and any(
        list_two == list_one[i : i + len_two] for i in range(len_one - len_two + 1)
    ):
