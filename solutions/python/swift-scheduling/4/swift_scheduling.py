from calendar import FRIDAY, SATURDAY, SUNDAY, WEDNESDAY
...
def asisoformat(func: Callable) -> Callable:
    return lambda *args, **kwargs: func(*args, **kwargs).isoformat()


@asisoformat
def delivery_date(start: str, description: str) -> dt:
    ...
    if description.endswith("M"):
        return _month(s, int(description.removesuffix("M")))

    if description.startswith("Q"):
        return _quarter(s, int(description.removeprefix("Q")))

    return s  # Unable to parse description
